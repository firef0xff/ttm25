...
uint16_t crc16_modbus( uint8_t const* data, size_t size )
{
...
{
    if ( size < mRequest.ResponseSize() )
        return false;

    uint8_t const* head = buf;
    uint8_t addr = 0;
    uint16_t crc = 0;
    head = FromBuff( addr, head );

    bool res = false;
    head += mRequest.Read( head, size - (head - buf), res );
    uint16_t kcrc = crc16_modbus( buf, head - buf );
    head = FromBuff( crc, head );
    res *= crc == kcrc;
    return res;
}
...
void Paskage::Build()
{
    ...
    mCrc = crc16_modbus( mOutBuf, head - mOutBuf );
